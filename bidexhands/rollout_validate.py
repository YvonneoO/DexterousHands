...
def main():
    ...
    for step in range(steps):
        ...
        if isinstance(infos, dict):
            for key in diagnostic_values:
                if key in infos:
                    values = as_numpy(infos[key])
                    diagnostic_values[key].extend(values.reshape(-1).tolist())
                    trace.setdefault(key, []).append(float(values.reshape(-1)[0]))
    ...
    for key, values in diagnostic_values.items():
        summary[key + "_mean"] = float(np.mean(values)) if values else None
        summary[key + "_max"] = float(np.max(values)) if values else None
        summary[key + "_p99"] = float(np.percentile(values, 99)) if values else None
